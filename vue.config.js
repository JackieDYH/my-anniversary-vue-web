 ...
 * @LastEditTime: 2023-10-17 21:15:29
 * @LastEditors: Jackie
...
module.exports = defineConfig({
  ...
  // 公共路径配置错误：请确保在项目的 vue.config.js 文件中正确配置了公共路径。公共路径应该与你在 GitHub Pages 上托管项目的路径相对应。例如，如果你的项目托管在 https://jackiedyh.github.io/my-anniversary-vue-web/，则公共路径应设置为 '/my-anniversary-vue-web/'。
  // 资源文件未正确部署：请确认在将项目打包后，已将生成的 dist 文件夹中的所有文件正确地上传到 GitHub 仓库的 gh-pages 分支或 docs 文件夹中。确保文件路径与资源请求路径一致。
  publicPath:
    process.env.NODE_ENV === 'production' ? '/my-anniversary-vue-web/' : '/',
  // 引入scss全局变量
  css: {
    loaderOptions: {
      sass: {
